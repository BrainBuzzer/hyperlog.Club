...
**Note:** List subtraction uses [strict comparison](https://hyperlog.club/resources/elixir/basics/) to match the values. For example:
...
In addition to the aforementioned functions, you can use [pattern matching](https://hyperlog.club/resources/elixir/pattern-matching/) and the cons operator `|` to split a list into head and tail. We'll learn more about this pattern in later lessons:
...
It is common for tuples to be used as a mechanism to return additional information from functions; the usefulness of this will be more apparent when we get into [pattern matching](https://hyperlog.club/resources/elixir/pattern-matching/):
